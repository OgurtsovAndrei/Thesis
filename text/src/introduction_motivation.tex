...
This thesis takes apart, simplifies, and speeds up the Goswami-style
...
suffixes outperforms WPS at LSM bucket sizes; eliminating WPS in turn
lets us collapse Goswami's two metadata vectors $D_1, D_2$ into a
single vector $D$.
Beyond the bit-count savings, the contiguous memory layout reduces pointer chasing and speeds up queries.
We arrive at the same backend structure as Grafite, with
the empirical evidence that this simplification is in fact warranted
(Chapter~\ref{chap:ere}).

On top of this backend we add a distribution-aware segmentation that
detects dense clusters and handles them exactly --- going below the
worst-case bound on real-world data (Search on Sorted Data
