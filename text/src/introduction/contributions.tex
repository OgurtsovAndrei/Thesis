...
and Memento adds dynamicity while matching Grafite on static workloads.
...
\begin{enumerate}
    ...
    suffixes outperforms it at the bucket sizes that arise at per-SSTable
    scale (Section~\ref{sec:eval-workload-params}), removing $59$~bits of
    ...
    On SOSD datasets this drives the FPR below $10^{-8}$ at
    $\sim\!11$ bits per key on Facebook User IDs and $3.7$ bits per key on
    ...
    the \texttt{select\_zero} pathology in the SUX
    succinct-data-structures library~\citep{vigna2008broadword} on clustered inputs
...
\begin{itemize}
    ...
    \item \textbf{Chapter~\ref{chap:conclusion}} summarises the results of the thesis.
\end{itemize}
