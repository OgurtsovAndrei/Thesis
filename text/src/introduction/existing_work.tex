...
encoding at about $10$ bits per node~\citep[\S3.1]{zhang2018surf}.
...
keys''~\citep[\S3.1]{zhang2018surf}, and the empirical FPR on the Yahoo! Cloud Serving Benchmark (YCSB)
...
key in a Bloom cascade --- one filter per prefix length,
...
power-of-two-aligned sub-intervals; for each one, Rosetta probes the Bloom filter at
...
cost''~\citep[\S2]{luo2020rosetta}. On a $50$M uint64-key benchmark
...
the advantage decreases on long ranges.
...
\log_2(1/\varepsilon)$ bits per key, which is lower than Goswami's
bound by $\log_2 \mathcal{L}$ bits per key: SNARF goes below
Goswami's bound when its assumptions about the data hold.
...
Memento~\citep{eslami2024memento} adds online insertions and
deletions that preserve the FPR guarantee for any
workload~\citep[\S1, \S4]{eslami2024memento}.
...
On static workloads Memento has the same memory usage as Grafite
but $1.5\times$ higher FPR~\citep[\S7.1, Fig.~9]{eslami2024memento}.
...
workload~\citep[\S6, Table~1]{eslami2024memento}. This matches
...
LSM-Tree deployment~\citep[\S1]{eslami2024memento}; Memento targets
