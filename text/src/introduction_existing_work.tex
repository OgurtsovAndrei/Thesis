...
$2 \log_2 R$ dyadic Bloom probes. The cascade is shown to approach
...
mitigated by allocating the per-level bit budget from a sample of
...
distribution-dependent: on the multi-modal OSM dataset SNARF itself
needs $15$--$16$ bits per key to reach FPR below
$10^{-4}$~\citep[\S6.1.2]{vaidya2022snarf}. SNARF additionally has no
...
all packed via Robin Hood displacement. Memento reports false-positive
...
suffice for the immutable SSTables of a per-file LSM
deployment~\citep[\S2]{eslami2024memento}; Memento targets workloads
