...
via two layers:
\begin{enumerate}
    \item A locality-preserving hash $h\colon U \to U'$, where $U' = \{0,1\}^K$ and $r = |U'| = 2^K$.
    The hash partitions $U$ into blocks of size $r$, applies an independent cyclic
    shift within each block, and then maps the shifted block onto $U'$ by translation.
    Its pairwise collision probability is at most $1/2^K$;
    see Section~\ref{sec:soda} for details.
