...
> The approximate version allows false positives but no false negatives — which enables us to makes it much smaller in memory."
...
> And on top of that, how we built the Segmented filter, which achieves better FPR than competing algorithms on some datasets like FB.
...
> Inserts go into a memtable in RAM; when it fills up,
...
> Because of sequential I/O, all insertions and all compactions are fast.
>
> On reads the picture is different:
> a lookup must scan every SSTable in the tree.
...
**Example text (~45s):**
> To avoid reading every SSTable from disk during a lookup,
> each file's metadata contains a filter.
> It answers with a one-sided error: no false negatives, 
> false positives are allowed.
> That is enough — if the filter says "the key is not present", the database skips the SSTable without any disk I/O.
> Making these filters probabilistic enables us to make them small.
...
> And suggested the stucture that reaches this lower limit up to a constant.
> As for the computational complexity, this data structure answers queries in constant time.
> 
> Their construction:
> - a locality-preserving hash that compresses the universe, and
> - an exact range emptiness filter that is constructed on a smaller universe answer range queries exactly
...
**Example text (~25s):**
> ER filter splits each key into a prefix and a suffix.
> Suffixes are grouped into buckets; prefixes drive the metadata that navigates to the right bucket.
> 
> Goswami stores metadata as two separate vectors, 
> We managed to collapse them into one, these gives us better mem layout.
...
>
> Look at the table, here in all these cases WPS does the same number of steps. 
> But on small bucket size — where WPS fits in L1, it works much faster.
>
> LMS designs limit SSTable sizes, so the merge time is limited and blast radius is limited.
> Thus, buckets sizes are also typically limited by thousands of keys.
> 
> At these bucket sizes Adaptive binary/linear search outperforms WPS by huge margin,
> with no auxiliary index required.
>
> Also, Goswami stored two vectors with metadata.
> I replaced them with a single contiguous vector.
...
> Now let's discuss how to choose a
> locality-preserving hash based on the data distribution.
>
...
> For example, S2 cell IDs - clustered around densely populated areas. 
> File paths, URLs, S2 cell IDs — dense segments alternate with sparse regions.
>
...
**Example text (~55s):**
> In a dense cluster: many keys in a narrow window.
> Approximate range filter **size** — is independent of U **size**.
> But, ERE cost — shrinks with the **size** of the Univ.
>
> Thus, if keys are dense enough, 
> It costs fewer bits to build ERE on the original keys, and this will give us zero false positive rate.
> This is the exact mode.
>
> "In sparse regions where we have very few points;
> We can simply truncate some lower bits to obtain our locality-preserving hash.
> And this hash will still have few collisions."
...
**Example text (~40s):** 
> Let me explain how to partition the keys.
> We apply **1D-DBSCAN**.
> **BSCAN** is a clustering algorithm that runs on sorted one-dimensional keys in linear time.
> The division threshold for it is derived from the filter parameters.
>
> On Each dense cluster we will build Exact filter.
> While, sparse gaps that DBSCAN marks as noise will go to the truncation fallback.
...
> we not only reach a lower FPR, but also we answer queries faster.
...
**Example text (~30s):**
> Honest limitation: filters are distribution dependent.
> On Uniform keys, point queries we do not win.
> No clusters to segment and no ranges to benefit from truncation.
> While, we pay the segmentation overhead.
...
**Example text (~50s):**
> Three main results.
> We optimized mem layout, reducing the metadata usage
