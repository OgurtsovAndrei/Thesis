...
def main() -> int:
    ...
    for mod in MODULES:
        ...
        for mode, detailed_reports in reports_by_mode.items():
            ...
            suffix = f"_{mode}" if mode != "default" else ""
            ...
            plotter.draw_stacked_area_chart(
                os.path.join(PLOTS_DIR, f"{mod['name']}{suffix}_mem_breakdown_bytes.svg"),
            ...
            plotter.draw_stacked_area_chart(
                os.path.join(PLOTS_DIR, f"{mod['name']}{suffix}_mem_breakdown_bits.svg"),
            ...
            # Export CSV
            csv_path = os.path.join(PARSED_DIR, f"{mod['name']}{suffix}_mem_breakdown.csv")
            with open(csv_path, "w", newline="") as f:
                ...
                for n in sorted_n:
                    ...
                    writer.writerow(row)
            print(f"Detailed CSV breakdown for {mod['name']} ({mode}) saved to {csv_path}")
