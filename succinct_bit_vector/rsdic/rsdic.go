...
func (rsd RSDic) MarshalBinary() ([]byte, error) {
	var buf bytes.Buffer
	for _, s := range [][]uint64{rsd.bits, rsd.pointerBlocks, rsd.rankBlocks, rsd.selectOneInds, rsd.selectZeroInds} {
		if err := writeUint64Slice(&buf, s); err != nil {
			return nil, err
		}
	}
	// rankSmallBlocks as []uint8
	if err := binary.Write(&buf, binary.LittleEndian, uint64(len(rsd.rankSmallBlocks))); err != nil {
		return nil, err
	}
	if _, err := buf.Write(rsd.rankSmallBlocks); err != nil {
		return nil, err
	}
	// scalar fields
	scalars := []uint64{rsd.num, rsd.oneNum, rsd.zeroNum, rsd.lastBlock, rsd.lastOneNum, rsd.lastZeroNum, rsd.codeLen}
...
// UnmarshalBinary decodes the RSDic from a binary form generated by MarshalBinary.
func (rsd *RSDic) UnmarshalBinary(in []byte) error {
	r := bytes.NewReader(in)
	var err error
	for _, sp := range []*[]uint64{&rsd.bits, &rsd.pointerBlocks, &rsd.rankBlocks, &rsd.selectOneInds, &rsd.selectZeroInds} {
		if *sp, err = readUint64Slice(r); err != nil {
			return err
		}
	}
	var n uint64
	...
	rsd.rankSmallBlocks = make([]uint8, n)
	if _, err := io.ReadFull(r, rsd.rankSmallBlocks); err != nil {
		return err
	}
	scalars := make([]uint64, 7)
	if err := binary.Read(r, binary.LittleEndian, scalars); err != nil {
		return err
	}
	rsd.num, rsd.oneNum, rsd.zeroNum = scalars[0], scalars[1], scalars[2]
	rsd.lastBlock, rsd.lastOneNum, rsd.lastZeroNum, rsd.codeLen = scalars[3], scalars[4], scalars[5], scalars[6]
	return nil
}
